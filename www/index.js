...
var app = {
    initialize: function() {
        console.log('in bind events index.js');
        this.bindEvents();
    ...
    checkForPassword: function() {
        var password = localStorage.getItem('appPsss22');
        if (password == null) {
    ...
    createNewPassword: function() {
        swal({	
            ...
            localStorage.setItem('appPsss22', inputValue);
            swal.close();
        });
    },	
    ...
    bindEvents: function() {
        alert('in bind events index.js');
        console.log('in bind events index.js');
        document.addEventListener('deviceready', this.onDeviceReady, false);
		document.addEventListener("deviceready", onDeviceReady, false);
        document.addEventListener('DOMContentLoaded', this.onDeviceReady);
        $("#searchtab").keyup(function() { // If nothing needed for this function ".keyup(null)"
    ...
    successCB: function(tx, results) {
        ...
        $('ul').children('li').on('click', function() {
            ...
            swal({
                ...
                var pwd = localStorage.getItem('appPsss22');
                if (pwd === inputValue) {
                    //alert("password matches");
                    //selectedIndex = $(this).index();
                    localStorage.setItem('incorrectpwd', 0);
                    //alert('Selected Index =' + selectedIndex + "\n");
                    window.location.href = "DetailsofNotes.html?msg=view" + selectedIndex;
                    swal.close();
                } else {
                    pwc++;
